...
@Component
public class KaspiCollectCategoryJobHandler implements JobHandler {

    private final KaspiCollector collector;
    private final RawFetchStore store;

    public KaspiCollectCategoryJobHandler(KaspiCollector collector, RawFetchStore store) {
    ...
    public boolean supports(String jobType) {
        return "COLLECT_CATEGORY".equals(jobType);
    }
    ...
    public void handle(JobQueueDao.ClaimedJob job) {
        ...
        var res = collector.collectCategory(url, page);
