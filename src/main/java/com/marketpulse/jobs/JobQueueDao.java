...
public class JobQueueDao {
  ...
  public void markSucceeded(UUID id) {
    jdbcTemplate.update(
        "update job_queue set status = 'SUCCEEDED', finished_at = now() where id = ?",
        id
    );
  }
  ...
  public void markDead(UUID id, String error) {
    jdbcTemplate.update(
        "update job_queue set status = 'DEAD', last_error = ?, finished_at = now() where id = ?",
        error, id
    );
  }
  ...
  public void markFailedAndRequeue(UUID id, String error, long delaySeconds) {
    jdbcTemplate.update(
        "update job_queue set status = 'QUEUED', last_error = ?, run_at = now() + (? || ' seconds')::interval, finished_at = now() where id = ?",
